...
class TestEvaluationText(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        print('Hello, we start our tests right now\n')
        options = {'classes': 2, 'shape': (25,), 'depth': 8}
...
def build_test_architecture(options):
    ...
    tmp_architecture.append(Block('embedding', layers_number=1, **options))
    tmp_architecture.append(Block('lstm'))
